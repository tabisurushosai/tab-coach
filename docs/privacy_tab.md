...
Console は `manifest.json` で宣言した権限ごとに個別の理由欄を表示する。本拡張機能は `permissions: ["tabs", "storage"]` の 2 権限のみを要求し、`host_permissions` は要求しない。各欄に以下のテキストを **完全一致** で貼り付ける (日英いずれも記入)。
...
| 言語 | 貼り付けテキスト |
| --- | --- |
| ja | 読了判定機能 (読み終わったタブを「読了」カテゴリに自動分類) のため、`document_idle` で軽量 content script を全 URL に注入します。本 script が取得・送信する情報はスクロール率 (数値)、滞在時間 (秒)、ページ URL、報告時刻のみで、ページ本文・フォーム入力・パスワード・Cookie・DOM 構造は **一切取得しません**。URL は読了判定キャッシュのキーとして `chrome.storage.local` でのみ使用し、外部送信しません。 |
| en | A lightweight content script is injected on `<all_urls>` at `document_idle` to support the "Read" auto-classification feature. The script sends only scroll ratio, dwell time, page URL, and reporting timestamp via `chrome.runtime.sendMessage`. Page body, form input, passwords, cookies, and DOM structure are **never accessed**. The URL is used only as the read-state cache key in `chrome.storage.local` and is never transmitted externally. |
...
本拡張機能は外部サーバー・開発者・第三者へユーザーデータを **収集しない (Collect = No)**。端末内で機能提供に使用するデータタイプは以下の表に従って申告する:
...
| ウェブ履歴 (Web history) | **使用する (Use のみ ON、Collect は OFF)** | `tabs` 権限で現在開いているタブの URL / タイトルを取得し、分類・整理・アーカイブ・読了判定キャッシュに端末内で使用する。Chrome の履歴データベース (`history` 権限) は読み取らない。 |
...
| `manifest.json` | `permissions` (2 件から増えていないこと)、`host_permissions` (未宣言であること) |
...
- [ ] §2.1 `tabs` 正当化文を貼付 (日英)
- [ ] §2.2 `storage` 正当化文を貼付 (日英)
- [ ] §2.3 `host_permissions` が未要求であることを Console 上で確認 (該当欄が表示されない)
- [ ] §4.1 の表に従い、外部収集は OFF、端末内使用するデータタイプは Use のみ ON で申告
- [ ] §4.2 必須証明 3 つすべて ✅ ON
- [ ] §4.3 アカウント機能 = なし
- [ ] §5 リモートコード = 使用しない (No)
...
- [ ] `manifest.json` の `permissions` が `["tabs", "storage"]` のままで、`host_permissions` が未宣言
